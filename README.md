...
## Position 6の画像そのものの直接調査、原因を作製工程に系統的な要因と判定（2026-09-16）

位置合わせの計算方法ではなく、Position 6の実画像内容そのものを他視野(1, 2, 5, 8)と
直接比較した。**判定は(b) 基板作製工程(パターン書き込みフィールド境界)に起因する
系統的なもの。** Position 6・7の視野には他視野より桁違いに明るい(一部65535に飽和する)
交差する直線状の構造があり(55000 DN以上の画素数がPosition 6で32315、次点のPosition 2は
2800)、これが**別日程・別基板(260828)でもほぼ同じ画素位置に再現した。** ピントの鋭さ
(Laplacian分散・Tenengrad)や格子検出率そのものはPosition 6が他視野より劣っておらず、
2026-08-26の基板製作時QCログにもSample 1・Position 6の欠陥記録はなかったため、
(a)目視で識別済みのゴミ・キズ、(c)撮影条件の悪化、はいずれも主因として支持されない。
この境界構造が、ECCの大並進局所解トラップとORB/RANSACの回転時並進バイアスの
両方を説明し得る。対策として、該当帯状領域(x≈16–130, y≈1780–1936)の人手承認マスク化と、
撮影直後の飽和画素数による自動QCフラグ追加を提案する。詳細は
[`docs/POSITION6_IMAGE_FORENSICS_20260916.md`](docs/POSITION6_IMAGE_FORENSICS_20260916.md) を参照。

## マスク実装・自動QCフラグ・標準方式の最終決定（2026-09-16）

`shared/image_qc.py`にPosition 6・7の明部帯を検出する`bright_band_mask`（座標
ハードコードなし、260826・260828の全16視野×pre/postで検証）と、撮影直後にフラグする
`saturation_qc`（閾値: 55000 DN以上18000画素、または完全飽和1500画素。32枚全件で
誤検出・見逃しゼロ）を実装した。

マスク適用後にECC・ORB/RANSACを実データ規模の並進・縦シフト・回転・拡大縮小で再比較した
結果、**ORB/RANSAC(マスク適用)が唯一、全シナリオでPosition 6の回収率100%を達成した。**
回転時の弱点(偽陽性率98.89%)はマスクで0.00%まで解消した。一方**ECCの大並進局所解
トラップはマスクでも解消しなかった**（回収率26.2%、マスクなしの33.9%よりむしろ悪化）。
これは書き込みフィールド境界ではなく、周期格子に対する相関最大化の捕捉範囲そのものの
限界と判断する。

**標準方式をORB/RANSACアフィン(マスク適用)に確定し、`shared/registration.py`の
Phase 2画像レベル位置合わせ呼び出し口(`register_image_pair_affine`)をこの方式に
差し替えた。** ただし本リポジトリの既存の生産コード(`pillar_level/v1`)は引き続き
Phase 1(溝検出+ICP点群位置合わせ)を呼んでおり、今回は変更していない。詳細・全シナリオの
