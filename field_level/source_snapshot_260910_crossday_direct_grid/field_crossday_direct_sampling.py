...
# This snapshot deliberately imports the colocated canonical-geometry snapshot,
# rather than whichever version happens to be in shared/ at runtime.
SNAPSHOT_SHARED = Path(__file__).resolve().parents[2] / "shared" / "source_snapshot_260910_canonical_geometry"
if str(SNAPSHOT_SHARED) not in sys.path:
    sys.path.insert(0, str(SNAPSHOT_SHARED))
